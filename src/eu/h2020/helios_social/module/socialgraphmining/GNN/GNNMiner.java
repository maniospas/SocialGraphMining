...
public class GNNMiner extends SocialGraphMiner {
	...
	/**
	 * Performs a fixed degradation of the learning rate over training epochs by multiplyng the latter
	...
	/**
	 * Sets the threshold at which old training examples are removed (default is 0.01).
	 ...
	 * with a random noise proportional to their norm. Zero values (default) ensure no privacy concerns but more exact 
	 * computations <i>for other</i> devices. The user's device would perform predictions depending on the privacy settings of
	 * their alters.
	 * 
	...
	public String getModelParameters(Interaction interaction) {
		...
		Node egoNode = context.getContextualEgoNetwork().getEgo();
		Tensor egoEmbedding = egoNode.getOrCreateInstance(GNNNodeData.class).getEmbedding();
		egoEmbedding = egoEmbedding
							.multiply(1-egoDeniability)
							.selfAdd(egoEmbedding.zeroCopy().setToRandom().multiply(egoEmbedding.norm()*egoDeniability));
		Tensor neighborEmbedding = aggregateNeighborEmbeddings(context);
		neighborEmbedding = neighborEmbedding
				.multiply(1-neighborDeniability)
				.selfAdd(neighborEmbedding.zeroCopy().setToRandom().multiply(egoEmbedding.norm()*neighborDeniability));
		return egoEmbedding.toString()+";"
			 + neighborEmbedding.toString()+";";
	}
